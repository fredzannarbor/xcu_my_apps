...
class SimpleAuth:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        if not st.session_state.get('authenticated', False):
            # Only try persistent login if we haven't already tried this session
            if not st.session_state.get('persistent_login_attempted', False):
                st.session_state.persistent_login_attempted = True
    ...
    def _initialize_session_state(self):
        ...
        st.session_state.persistent_login_attempted = False
    ...
    def _check_persistent_login(self):
        ...
        try:
            if not self.persistent_auth_file.exists():
                logger.debug("No persistent auth file found")
                return
            ...
            if datetime.now() - saved_time > timedelta(days=30):
                # Expired, remove file
                logger.info("Persistent credentials expired, removing file")
            ...
            logger.debug(f"Attempting auto-login for user: {username}")
            if username and password and self._authenticate_direct(username, password):
                logger.info(f"Successfully auto-logged in user {username} from persistent credentials")
                return True  # Successfully logged in
            else:
                logger.warning(f"Failed to auto-login user {username}, removing invalid credentials")
                self.persistent_auth_file.unlink()

        except Exception as e:
            logger.error(f"Error checking persistent login: {e}")
    ...
    def render_login_form(self) -> bool:
        ...
        if self.persistent_auth_file.exists():
            if self.is_authenticated():
                st.info("🔑 You have saved login credentials and are already logged in.")
                ...
                # Show that we have saved credentials and attempt auto-login
                st.info("🔑 You have saved login credentials. They will be used automatically.")

                # Try auto-login
                if self._check_persistent_login():
                    st.success("🎉 Automatically logged in with saved credentials!")
                    st.rerun()
                    return False
                else:
                    # Auto-login failed - credentials are invalid
                    st.warning("⚠️ Saved credentials are invalid or expired. Please log in manually.")
                    if st.button("🗑️ Clear Invalid Credentials"):
                        self._clear_persistent_credentials()
                        st.success("Invalid credentials cleared!")
                        st.rerun()
                        return False
