        ...
        if '(' in paragraph.text and ')' in paragraph.text:  # check if there is a parenthesis in the text of the current paragraph object, then add it to the list of acronyms using append() method.  If you don't find any parenthesis, skip to next step with continue statement below.  
            ...
            for paragraph in document.paragraphs:          # loop over each paragraph object within doc_obj's paragraphs attribute and check if any of these objects' text attributes contain the string stored in acronym variable...
                ...
                if acronym in paragraph.text:              # ...if there is a match, then add the first word from that paragraph object's text attribute (i.e., p_obj) to list of acronyms using .append() method so we can return this list as our function output...
                    ...
                    acronyms.append(paragraph.text[0])     # append first word from current p_obj (i.e., current paragraph) within doc_obj's paragraphs attribute to list of acronyms 
...
def change_hyphens_to_endashes_in_text(document):
    text = docx2txt.process(docfile)
...
def unmatched_quotes(document):
    ...
    for paragraph in document.paragraphs:
        if re.search('[“”]', paragraph.text) and not re.search('[“]', paragraph.text) or \
                not re.search('[”]', paragraph.text) and re.search('[“]', paragraph.text):
...
def find_wrong_terms(document):
    ...
    with open('../data/editorsguide.csv', 'r') as guide:
...
if __name__ == '__main__':
    cwd= os.getcwd()
    print(cwd)
    docfilepath = "../data/test.docx"
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument('--docfile', help='docx file to be converted', default=docfilepath)
    args = parser.parse_args()
    docfile = args.docfile

    # delete ../data/word_freq_data.csv
    if os.path.exists("../data/word_freq_data.csv"):
        os.remove("../data/word_freq_data.csv")
    ...
    acronyms = findall_acronyms(document)
    ...
    for word, number_of_usages in sorted(word_frequencies.items()):
        with open('../data/word_usage_freq.csv', 'a') as f:
            writer = csv.writer(f)
            f.write("{},{}\n".format(word, number_of_usages))
    
    print(*styles, sep='\n', file=open('../data/styles.txt', 'w'))
    print(*fonts_list, sep='\n', file=open('../data/fonts.txt', 'w'))
    print(*acronyms, sep='\n', file=open('../data/acronyms.txt', 'w'))


    print(*hyphen_sentences, sep='\n', file=open('../data/hyphen_sentences.txt', 'w'))
    print(*endash_sentences, sep='\n' , file=open('../data/endash_sentences.txt', 'w'))
    print(*quotes_with_unmatched_quotes, sep='\n', file=open('../data/quotes_with_unmatched_quotes.txt', 'w'))
