...
from app.utilities.files2llamaindex import check_if_index_dir_exists, summarize_index, load_or_create_simplevectorindex, \
    load_or_create_gptlistindex, query_simple_vector_index
...
from app.utilities.metadatas2output_schemas import metadatas2docx, metadatas2internationaledition, metadatas2bookjson
from app.utilities.metadatas_df2distributor_reqts import book_metadata_json2distributor_format_targets, \
...
def bulkprocessPDFs(pdf_directory, output_dir, list2string, cumulative_file_name, limit, profiling, payservices, engine,
                    ...
                    batch_limit, production_specs_filepath, skip_spec_check=False):
    ...
    try:
        specs_input_df.to_csv('output/job_results/' + short_uuid + '_metadata_stub_for_production_specs.csv',
    ...
    if profiling:
        ProfileReport(cumulative_df).to_file(
            "output/bulkpdfprocess_profile_report.html"
        )
        print(
            "Profile report is available at",
            "output/bulkpdfprocess_profile_report.html",
        )
    else:
        print("not generating profile report")
        # print loop for 10 seconds
        # st.write(results)
    # st.write('in bulkpdfprocess, cumulative_df is')
    # st.dataframe(cumulative_df.T)
    exit()
...
def process_pdf(filename, output_dir, limit, list2string, filecount, timestamped_filename, payservices, config,
    ...
    deduped_keywords, deduped_keywords_with_scores, keywords_text_list = create_and_save_keyword_lists(text, thisdoc_dir)
    ...
    if payservices:
        ...
        if hack:
            if run_recursively:
                ...
                try:
                    ...
                    tokens_in_recursive_synopsis = len(encoding35.encode(recursive_synopsis))
                ...
                print("using extractive synopsis, no recursive")
            ...
            try:
                print(type(submit_synopsis))
                print(submit_synopsis)
                print('length of submit synopsis', len(submit_synopsis))
            except Exception as e:
                print('error in printing submit synopsis', e)
        ...
        if text2images_generate_images:
            ...
            try:
                DALLE2_Mood_response = create_DALLE2_images(metadatas['Text2MoodImagePrompt'][1], n=3, dimensions="256x256", helicone_proxy=True)
            except Exception as e:
...
if __name__ == "__main__":
    ...
    (
        ...
        batch_limit, production_specs_filepath, add_to_final
    ) = argparse_handler()
