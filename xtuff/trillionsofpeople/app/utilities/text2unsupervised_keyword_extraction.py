...
def create_keyword_list_yake(text, output_dir="app/data/keyword_lists", language="en", max_ngram_size=3,
    ...
    print('automatically calculating goal number of index keywords', calculate_number_of_target_keywords)
    if calculate_number_of_target_keywords == True:
        ...
        print('using number of keywords passed into function', numOfKeywords)
    try:
        custom_kw_extractor = yake.KeywordExtractor(lan=language, n=max_ngram_size, dedupLim=0.9,
                                                    dedupFunc=deduplication_algo, windowsSize=windowSize,
                                                    top=numOfKeywords, features=None)
        keywords = custom_kw_extractor.extract_keywords(text)
        print('custom kw extractor was successful')
    except Exception as e:
        print('error in custom keyword extraction', e)

    print('about to loop through keywords')
    for count, kw in enumerate(keywords):
        if count % 20 == 0:
            print(count, 'keywords generated')
    ...
    print(str(len(kw_only_list)), 'keywords created in first pass')

    deduped = process.dedupe(kw_only_list, threshold=80)
    print(str(len(deduped)) + ' after deduping')
    # print(deduped)
    deduped_with_scores = {}
    for d in deduped:
