...
def common_presets_for_metadata():
    common_presets = {'TitleFromDescription': 'Title & Subtitle',  'BibliographicKeywordsGenerator': 'Bibliographic Keywords', 'BookDescriptionWriter': 'Book Detail Page Description', 'BISAC_CategoryRecommender': "BISAC Categories", "BlurbWriter": "Back Cover Blurb Writer", }
...
def create_title_metadata_using_openai(summary_sentences_string, output_dir='output', engine="text-curie-001"):
    ...
    KDP_presets = ["TitleFromDescription", "BibliographicKeywordsGenerator", "BookDescriptionWriter", "BISAC_CategoryRecommender", "BlurbWriter"]

    KDP_results_df = pd.DataFrame({'Title Ideas': pd.Series(dtype='str'),
                   'Bibliographic Keywords': pd.Series(dtype='str'),
                   'Description': pd.Series(dtype='str'),'BISAC categories': pd.Series(dtype='str'),'Blurb': pd.Series(dtype='str'),})
...
def create_series_metadata_using_openai(summary_sentences_string):
    ...
    if new_series:
        if main_content == 'Yes':
            ...
            series_dict = {'SeriesTitleOnlyGenerator': 'Series Title Ideas', 'SeriesTitleandDescriptionGenerator': 'Series Title & Description  Ideas', 'SeriesSequelTitles': 'Series Sequel Ideas', 'SeriesDescriptionGenerator': 'Series Description'}
            series_presets = ["SeriesTitleOnlyGenerator", 'SeriesDescriptionGenerator',"SeriesTitleandDescriptionGenerator"]
            if series_presets:
