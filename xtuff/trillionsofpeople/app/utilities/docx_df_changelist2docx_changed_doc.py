...
def setup(df_filename, docx_filename):  # , basedocx):
    ...
    st.info('docx_df_changelist2docx_changed_doc.py')
    ...
    if num_rows != number_paragraphs:
        print('number of rows in df {}'.format(num_rows) + 'does not match number of paragraphs in doc {}'.format(number_paragraphs))    
        print('Automatic editing may be unreliable.  Make sure the rows in the dataframe match the paragraphs in the document.')
        exit()
    else:
...
def parse_requested_style(requested_style):
    try:
        actions_dict = {'style2bodytext':'Body Text', 'style2caption':'Caption', 'style2bibliography':'Bibliography', 'style2byline':'Byline', 'style2title':'Title', 'style2normal':'Normal', 'style2quote': 'Quote', 'style2quotesignature' : 'quote signature', 'style2heading1': 'Heading 1', 'style2part': 'Part', 'style2signature': 'Signature'}
        ...
        for count, p in enumerate(doc.paragraphs):
            if p.text == para_text:
                ...
                if isinstance(requested_action, str):
                    if requested_action.startswith("style2"):
                        try:
                            ...
                            print('applying style {} to paragraph {} that begins with {}'.format(requested_style, count, para_text[0:20]))
                        except Exception as e:
                ...
                if para_text.startswith('Document_Image') and next_para_text == '':
                    ...
                    print('applying style Caption to paragraph {} that follows Document_Image in previous paragraph {}'.format(count + 1 , count))
...
def argparse_handler():
    ...
    argparser.add_argument("--basedocx", help = "base Word file", default = "/Users/fred/bin/nimble/unity/editorial/template.dotx")
    argparser.add_argument('--output_dir', help='path to output directory', default='output')
    argparser.add_argument('--df_filename', help='output converted text as single string, not a list', default='test/csv/lorem2.csv')
