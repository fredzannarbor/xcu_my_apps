...
class Models2BookIdeas:
    ...
    api_type: str = "ollama", base_url: str = "https://api.x.ai/v1", api_key_host_name = "XAI_API_KEY", ollama_host: str = "http://localhost:11434"):
        """
        ...
        self.api_key_host_name = api_key_host_name
        self.ollama_host = ollama_host.rstrip('/')
        self.api_url = f"{self.ollama_host}/api/generate"  # Construct full Ollama API URL
        if self.api_type == "openai":
            load_dotenv()
            api_key = os.getenv(api_key_host_name, "no api key found")
            base_url = base_url
            self.client = openai.OpenAI(api_key=api_key, base_url=base_url)
        self.logger.info(f"{model.__repr__()}")
        # initialize prompts
    ...
    def _create_with_openai_style_client(self, prompt, model, temperature):
        ...
        try:
            ...
            self.logger.debug(f"Response: {string_response}")
            json_response = json.loads(string_response)
            return json_response #
            # string_response

        except Exception as e:
            self.logger.error(f"Error generating ideas with OpenAI-style client: {traceback.format_exc()}")
            return []
    ...
    def _create_with_ollama(self, prompt, model, temperature):
        ...
        try:
            ...
            #logger.debug(f"Response {string_response}")
            logger.debug(string_response)
            json_response = json.loads(string_response)
            return json_response

        except Exception as e:
            logger.error(f"Error in creating Ollama: {traceback.format_exc()}")
            return []
    ...
    def expand_ideas_batch(self, ideas: List[str], num_variations: int = 5, prompt_template: str = None) -> Dict[
        ...
        for idea in tqdm(ideas, desc="Expanding ideas", unit="idea"):
            logger.debug(f"Processing idea: {idea}, Variations: {num_variations}, Prompt: {prompt_template}")
            results[idea] = self.expand_idea(idea, num_variations, prompt_template)
            logger.debug('---')
            logger.debug(results[idea])
            logger.debug('---')
        return results
    ...
    def save_to_csv(self, seed_dict: Dict[str, List[str]], output_file: str):
        try:
            ...
            logger.info(f"Saved CSV file to: {output_file}")
...
if __name__ == "__main__":
    ...
    if args.batch:
        try:
            ...
            if not ideas:
                logger.error(f"No ideas found in {args.batch}")
                exit(1)
            ...
            logger.error(f"Error: File '{args.batch}' not found.")
            exit(1)
        ...
        logger.error("Error: Please provide either an idea or a batch file path.")
        exit(1)
