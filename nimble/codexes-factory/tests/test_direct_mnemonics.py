#!/usr/bin/env python3
"""
Test script to directly test the mnemonics prompt without using the prompt manager.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Test direct mnemonics prompt")
    ...
    # Save the mock LLM call if requested
    if args.output:
        try:
            mock_output_path = args.output + ".mock_llm_call.json"
            with open(mock_output_path, 'w', encoding='utf-8') as f:
                json.dump(mock_llm_call, f, indent=2)
            logger.info(f"Saved mock LLM call to {mock_output_path}")
        except Exception as e:
            logger.error(f"Error saving mock LLM call to {mock_output_path}: {e}")
