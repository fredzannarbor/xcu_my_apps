...
def create_rows_for_picturebook(text, n=1, image_generators=["dalle"], size='256x256'):
    rows = []
    if "replicate-mj" in image_generators:
        import app.utilities.text2midjourney_clone as text2midjourney
    ...
    for line in text.splitlines():
        ...
        meta_prompt = line
        image_prompt_text = chatcomplete("ImagePromptGenerator", meta_prompt, "gpt-3.5-turbo", '37')
        submit_prompt = image_prompt_text + ' Use the visual style of Ludwig Bemelmans.'
        ...
        for generator in image_generators:
            ...
            if generator == "dalle":
                ...
                image = text2midjourney.run_midjourney_clone(inputs)
                print(image)
                image_url = image.url
        ...
        prompt_row = {"string": image_prompt_text, "style": "Caption"}
...
def rows2docx(rows, thisdoc_dir):
    ...
    try:
        ...
        print(errmessage)
    uuid_string = str(uuid.uuid4())
    booktargetpath = thisdoc_dir + '/' + uuid_string + '_newbook.docx'
    try:

        document.save(booktargetpath)
        print('saved', booktargetpath)
    except Exception as e:
        errmessage = str(e) + 'error in saving word doc'
        print(errmessage)
...
if __name__ == "__main__":
    rows = create_rows_for_picturebook(text=text, image_generators=["dalle"])
